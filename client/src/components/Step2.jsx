...
function Step2({ interviewData, onFinish }) {
  ...
  useEffect(() => {
    const loadVoices = () => {
      ...
      if (voices.length === 0) {
        ...
        // If after a short timeout still no voices, use fallback to unblock
        setTimeout(() => {
          if (window.speechSynthesis.getVoices().length === 0 && isMountedRef.current) {
            console.warn("⚠️ No voices found, using default system voice (fallback mode)");
            setSelectedVoice(null); // speech synthesis will use default
      ...
      if (isMountedRef.current) {
        ...
        setVoicesLoaded(true);
        setStartError("");
        console.log(`✅ Selected voice: ${finalVoice?.name || "(default)"}`);
      }
    };
  ...
  // ============================================================================
  // HELPER: Request Fullscreen Mode
  // ============================================================================
...
const startInterview = useCallback(async () => {
  if (interviewStarted || isStartingRef.current) return;
  ...
  try {
    ...
    if (!fullscreenSuccess) {
      setStartError("Please enter Full Screen mode to start the interview.");
      return;
    }
    ...
    if (!navigator.mediaDevices?.getUserMedia) {
      setStartError("Microphone is not supported in this browser.");
      return;
    }
    ...
    if (!questions?.length || !questions[0]?.question) return;
    ...
    try {
      ...
      const result = await axios.post(
        `${ServerUrl}/api/interview/submit-answer`,
        {
          ...
          answer,
          timeTaken: elapsedTime, // better to use elapsedTime since we track it directly!
        },
        {
          withCredentials: true,
        }
  ...
  }, [answer, currentIndex, currentQuestion, finishInterview, interviewId, isSubmitting, questions, speakText, timeLeft, elapsedTime, totalQuestions, stopRecognition, startRecognition, clearTranscript]);
